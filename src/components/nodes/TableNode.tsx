          ...
          {(testCases.length > 0 ? testCases : [createNewTestCase()]).map((tc, index) => (
            <div key={index} className="p-3 border rounded-lg bg-card">
              ...
              <div className="grid grid-cols-8 gap-2">
                <div><Label className="text-xs">ID (*Required)</Label><Input value={tc.id} onChange={(e) => updateTestCaseField(index, 'id', e.target.value)} className="text-xs nodrag nopan" /></div>
                <div><Label className="text-xs">Test Case</Label><Input value={tc.testCase} onChange={(e) => updateTestCaseField(index, 'testCase', e.target.value)} className="text-xs nodrag nopan" /></div>
                <div><Label className="text-xs">Category</Label><Input value={tc.category} onChange={(e) => updateTestCaseField(index, 'category', e.target.value)} className="text-xs nodrag nopan" /></div>
                <div><Label className="text-xs">Exploited</Label><Select value={tc.exploited} onValueChange={(v) => updateTestCaseField(index, 'exploited', v)}><SelectTrigger className="text-xs nodrag nopan"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Yes">Yes</SelectItem><SelectItem value="No">No</SelectItem></SelectContent></Select></div>
                <div><Label className="text-xs">URL</Label><Input value={tc.url} onChange={(e) => updateTestCaseField(index, 'url', e.target.value)} className="text-xs nodrag nopan" /></div>
                <div className="col-span-1"><Label className="text-xs">Evidence</Label><div className="space-y-1 mt-1 max-h-20 overflow-y-auto">{tc.evidence.map((ev, evIndex) => (<div key={evIndex} className="flex items-center justify-between bg-background p-1 rounded text-xs" title={ev.path}><span className="truncate">{ev.name}</span><Button size="icon" variant="ghost" className="h-5 w-5 shrink-0" onClick={() => removeEvidence(index, evIndex)}><X className="h-3 w-3"/></Button></div>))}</div><Button size="sm" variant="outline" className="w-full mt-2 text-xs nodrag nopan" disabled={!tc.id.trim()} onClick={() => fileInputRefs.current[index]?.click()}><FileUp className="h-3 w-3 mr-1"/> Upload</Button><div tabIndex={0} onPaste={(e) => handlePasteEvidence(index, e)} className="w-full mt-1 border border-dashed rounded text-[10px] text-muted-foreground text-center py-1 nodrag nopan focus:outline-none focus:ring-2 focus:ring-primary cursor-text" title="Click then Ctrl+V to paste image">Paste Image (Ctrl+V)</div><input ref={el => fileInputRefs.current[index] = el} type="file" multiple accept="image/*" className="hidden" onChange={(e) => e.target.files && handleEvidenceUpload(index, e.target.files)} /></div>
                <div><Label className="text-xs">Status</Label><Select value={tc.status} onValueChange={(v) => updateTestCaseField(index, 'status', v)}><SelectTrigger className="text-xs nodrag nopan"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Pass">Pass</SelectItem><SelectItem value="Fail">Fail</SelectItem><SelectItem value="Not Applicable">Not Applicable</SelectItem></SelectContent></Select></div>
                <div><Label className="text-xs">Tester</Label><Input value={tc.tester} onChange={(e) => updateTestCaseField(index, 'tester', e.target.value)} className="text-xs nodrag nopan" /></div>
              </div>
            </div>
          ))}
        </div>
